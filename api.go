...
// GetCertificates is a stb implementation for cert info
func (c *Client) GetCertificates() ([]Certificate, error) {
	r := c.NewRequest("GET", "/director/certificate_expiry")
	resp, err := c.DoRequest(r)

	if err != nil {
		log.Printf("Error requesting certificate info  %v", err)
		return nil, err
	}
	defer resp.Body.Close()
	...
	resBody, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		log.Printf("Error reading certificate info %v", err)
		return nil, err
	}
	certificates := []Certificate{}
	err = json.Unmarshal(resBody, &certificates)
	if err != nil {
		log.Printf("Error Unmarshalling certificates %v", err)
		return nil, err
	}
