...
var _ = Describe("Api", func() {
	Describe("Test API", func() {
		...
		Describe("Test get certificates", func() {
			BeforeEach(func() {
				setup(MockRoute{"GET", "/director/certificate_expiry", certificates, ""}, "basic")
				config := &Config{
		...
		Describe("Test get releases", func() {
			...
			It("can get releases", func() {
				...
				Expect(err).Should(BeNil())
				Expect(releases[0].Name).Should(Equal("bosh2"))
